	...
	public static ObjectNode getJoinedChatgroupsForIMUser(String username) {
		...
		try {
			...
			webTarget = EndPoints.USERS_TARGET.resolveTemplate("org_name", APPKEY.split("#")[0])
					.resolveTemplate("app_name", APPKEY.split("#")[1]).path(username).path("joined_chatgroups");
			//"https://a1.easemob.com/easemob-demo/chatdemoui/users/kenshinn/joined_chatgroups"
			objectNode = JerseyUtils.sendRequest(webTarget, null, credential, HTTPMethod.METHOD_GET, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
